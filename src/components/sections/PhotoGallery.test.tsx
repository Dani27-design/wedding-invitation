...
describe('PhotoGallery', () => {
  ...
  describe('images', () => {
    ...
    it('all images have lazy loading attribute', () => {
      renderGallery();
      const images = screen.getAllByAltText(/Gallery/);
      images.forEach((img) => {
    ...
    it('images have alt text "Gallery N" for each index', () => {
      renderGallery();
      for (let i = 0; i < 12; i++) {
    ...
    it('images have w-full h-full to fill container', () => {
      renderGallery();
      const images = screen.getAllByAltText(/Gallery/);
      images.forEach((img) => {
  ...
  describe('interaction', () => {
    ...
    it('clicking multiple images in sequence calls onSelectPhoto correctly', () => {
      const onSelectPhoto = vi.fn();
      renderGallery(onSelectPhoto);
      const img0 = screen.getByAltText('Gallery 0').closest('.cursor-zoom-in')!;
      const img5 = screen.getByAltText('Gallery 5').closest('.cursor-zoom-in')!;
  ...
  describe('performance', () => {
    ...
    it('all images use lazy loading for performance', () => {
      renderGallery();
      const images = screen.getAllByAltText(/Gallery/);
      const lazyCount = images.filter((img) => img.getAttribute('loading') === 'lazy').length;
      expect(lazyCount).toBe(12);
    });
  });
