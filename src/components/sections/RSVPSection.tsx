              ...
              <div className="h-full grid grid-cols-1 gap-1 md:grid-cols-2 content-start">
                <AnimatePresence mode="popLayout">
                  {currentWishes.map((wish, idx) => (
                    <motion.div key={wish.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.25, delay: idx * 0.02 }} className="w-full bg-white/60 px-3 py-1 rounded-xl border border-gold/10 border-l-2 border-l-gold/30 relative group hover:border-gold/20 transition-all shadow-sm">
