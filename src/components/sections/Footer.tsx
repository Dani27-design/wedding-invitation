...
import { Heart, Code, Palette, Instagram, Linkedin, LucideIcon } from 'lucide-react';
...
const SocialLink = ({ href, children }: { href: string; children: ReactNode }) => (
  <motion.a href={href} target="_blank" rel="noopener noreferrer" whileHover={{ y: -3, color: 'var(--color-gold)' }} className="text-ink">{children}</motion.a>
);
...
export const Footer = () => {
  ...
  const creditSocials = wedding ? [
    [
      { href: wedding.groomInstagram, Icon: Instagram },
      { href: wedding.groomLinkedin, Icon: Linkedin },
      { href: deriveWhatsappUrl(wedding.groomWhatsapp), Icon: WhatsAppIcon },
    ],
    [
      { href: wedding.brideInstagram, Icon: Instagram },
      { href: wedding.brideThreads, Icon: ThreadsIcon },
      { href: deriveWhatsappUrl(wedding.brideWhatsapp), Icon: WhatsAppIcon },
    ],
  ] : [];
          ...
          {wedding?.credits.map((credit, i) => {
            ...
            return (
              <div key={credit.name} className="p-2 md:p-3 rounded-[1.5rem] md:rounded-[2.5rem] bg-paper/50 border border-gold/5 flex flex-col items-center justify-evenly">
                ...
                <h5 className="font-serif italic text-lg text-ink mb-1 md:mb-2">{credit.name}</h5>
                ...
                <div className="flex gap-4 opacity-30 hover:opacity-100 transition-opacity">
                  {creditSocials[i]?.map(({ href, Icon }) => (
                    <SocialLink key={href} href={href}><Icon className="w-4 h-4" /></SocialLink>
                  ))}
                </div>
              </div>
            );
