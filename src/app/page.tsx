...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-ivory text-ink overflow-x-hidden">
      ...
      {/* ── Hero ── */}
      <section className="relative min-h-screen-safe h-full flex flex-col items-center justify-center px-6 py-auto bg-ink text-ivory overflow-hidden">
        ...
        <div className="relative z-10 max-w-lg mx-auto text-center space-y-6">
          ...
          <div className="relative flex justify-center pt-4 pb-2">
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[260px] h-[260px] border border-gold/10 rounded-full animate-[spin_45s_linear_infinite]" />
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] border border-ivory/5 rounded-full animate-[spin_60s_linear_infinite_reverse]" />
            ...
            <Link
              ...
              className="relative block w-[180px] rounded-[20px] overflow-hidden border-2 border-ivory/15 shadow-2xl shadow-gold/20 hover:scale-[1.03] hover:border-gold/30 transition-all duration-500"
              style={{ height: 320 }}
            >
              <iframe
      ...
      {/* ── Features ── */}
      <section className="px-6 p-6 bg-paper/30 border-y border-gold/5">
              ...
              <div key={label}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {items.map(({ icon: Icon, title }) => (
                    <div key={title} className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-white/60 border border-gold/5">
                      <Icon className="w-3.5 h-3.5 text-gold/60 flex-shrink-0" />
                      <span className="text-[11px] text-ink/60">{title}</span>
                    </div>
                  ))}
                </div>
              </div>
