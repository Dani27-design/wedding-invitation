...
vi.mock('../../context/WeddingContext', () => ({
  useWeddingContext: () => ({
    ...
    quranArabic: 'Verse line one\nVerse line two',
    quranTranslation: 'Dan di antara tanda-tanda kekuasaan-Nya ialah Dia menciptakan untukmu pasangan hidup.\nSupaya kamu merasa tenteram kepadanya.',
...
describe('EventSection', () => {
  ...
  describe('Content', () => {
    ...
    it('preserves original quote line breaks in the invitation', () => {
      renderComponent();
      const originalText = screen.getAllByText(/Verse line one/)[0];
      const translation = screen.getAllByText(/Supaya kamu merasa tenteram/)[0];

      expect(originalText).toHaveClass('whitespace-pre-line');
      expect(originalText.textContent).toContain('\n');
      expect(translation).toHaveClass('whitespace-pre-line');
      expect(translation.textContent).toContain('\n');
    });

    it('uses automatic text direction for flexible original quote languages', () => {
      renderComponent();
      const originalText = screen.getAllByText(/Verse line one/)[0];

      expect(originalText).toHaveAttribute('dir', 'auto');
    });
  });
