...
export const EventSection = memo(() => {
  ...
  <section id="event-section" className="relative py-[3vh] lg:py-20 bg-paper overflow-hidden">
    ...
    <div className="lg:hidden container mx-auto px-6 max-w-lg relative z-10 flex flex-col items-center text-center gap-[3vh]">
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.3 }} className="w-full flex flex-col items-center justify-center gap-[1vh]">
          {wedding?.ceremonies.map((c, i) => (
            <motion.div key={c.name} whileHover={{ y: -3, scale: 1.01 }} className={`w-full bg-ivory border border-gold/15 ${CEREMONY_SHAPES[i % CEREMONY_SHAPES.length]} py-3 px-6 text-center cursor-default transition-all shadow-sm`}>
          ...
          <motion.div whileHover={{ y: -3, scale: 1.01 }} className="w-full bg-ivory border border-gold/15 rounded-[2rem_3rem_3rem_2rem] py-3 px-6 text-center cursor-default transition-all shadow-sm">
    ...
    {/* Desktop: Left-Right Split (Option B) */}
    <div className="hidden lg:block container mx-auto px-12 xl:px-20 max-w-5xl relative z-10">
      <div className="flex gap-12 xl:gap-16 items-center min-h-[60vh]">
        ...
        {/* Right: ceremonies + venue + buttons */}
        <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.2 }} className="flex-1 max-w-[420px] flex flex-col gap-4">
          {wedding?.ceremonies.map((c, i) => (
            <motion.div key={c.name} whileHover={{ y: -4 }} className={`bg-ivory border border-gold/15 ${CEREMONY_SHAPES[i % CEREMONY_SHAPES.length]} py-7 px-8 text-center cursor-default transition-all shadow-sm`}>
          ...
          <motion.div whileHover={{ y: -4 }} className="bg-ivory border border-gold/15 rounded-[2rem_3rem_3rem_2rem] py-7 px-8 text-center cursor-default transition-all shadow-sm">
            <div className="flex justify-center items-center gap-2 mb-2">
              <MapPin className="w-4 h-4 text-gold" />
              <p className="font-serif italic text-lg text-ink">{wedding?.venueName}</p>
            </div>
            <div className="w-8 h-px bg-gold/30 mx-auto mb-2" />
            <p className="text-sm text-ink/60 font-light leading-relaxed">{wedding?.venueAddress}</p>
          </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.5 }} className="flex justify-center gap-4">
        <motion.a whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} href={safeUrl(wedding?.venueMapsUrl ?? '')} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 py-3 px-8 bg-gold text-ivory rounded-full text-xs uppercase tracking-[0.3em] font-black transition-all shadow-md">
          <MapPin className="w-3 h-3" />
          <span>Lihat Peta</span>
        </motion.a>
        <motion.button
